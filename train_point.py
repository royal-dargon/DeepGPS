...
from model.main_model import MainModel, AllModel, PointModel
...
def main(args):
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        run = wandb.init(
            ...
            entity='jingzhel_cn',
            config=vars(args),
            name="point_img_3e5_ExponentialLR_16",
        )
        wandb.watch(model)
    ...
    for epoch in range(1, args.epochs_num+1):
        ...
        for i, (seq, lab, nuc, ptp, points) in tqdm(enumerate(TrainLoader)):
            ...
            lab = lab.float()
            # 包含分类结果
            loss, _, outputs = model(x_img = nuc, y_lab = lab, y_img = ptp, p = points)
            loss.backward()
            optimizer.step()
        ...
        for j, (seq, lab, nuc, ptp, points) in tqdm(enumerate(ValidLoader)):
            ...
            with torch.no_grad():
                loss, res_cls, logits_img = model(x_img=nuc, y_lab=lab, y_img=ptp, p=points)
